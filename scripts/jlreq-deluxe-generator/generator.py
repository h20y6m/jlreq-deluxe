...
from vf import CharacterPacket, FontDefinition, SimpleDvi, VF
...
def generate(j_name, o_name, d_name):
    # jlreqのJFMを読み込み
    ...
    for k in kanji_list:
        ...
        if j_wd != o_wd:
            logger.debug(f"H {k:04X} CHARWD {to_fixed_str(j_wd)} <=> {to_fixed_str(o_wd)}")
            move_right = 0.0
            if j_wd < o_wd:
                if (
                    k in j_vf.char_packets
                    and isinstance(j_vf.char_packets[k].dvi, SimpleDvi)
                    and j_vf.char_packets[k].dvi.move_right is not None
                ):
                    move_right = j_vf.char_packets[k].dvi.move_right
                ...
                if (
                    k in o_vf.char_packets
                    and isinstance(o_vf.char_packets[k].dvi, SimpleDvi)
                    and o_vf.char_packets[k].dvi.move_right is not None
                ):
                    move_right = o_vf.char_packets[k].dvi.move_right
    ...
    with open(f"{d_name}.jpl", "w") as writer:
        d_jfm.dump(writer)

    with open(f"{d_name}.vpl", "w") as writer:
        d_vf.dump(writer)

    d_jfm_name = f"{d_name}.tfm"
    logger.info(f"Save {d_jfm_name}")
    d_jfm.save(d_jfm_name)

    d_vf_name = f"{d_name}.vf"
